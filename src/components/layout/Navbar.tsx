  ...
  return (
    ...
    >
      ...
      <div className="max-w-7xl mx-auto flex items-center justify-between">
          ...
          {navLinks.map((link) => (
            ...
            >
              <Link
                href={link.href}
                className={cn(
                  "font-medium transition-colors hover:text-medical flex items-center gap-1 py-4 text-sm xl:text-base",
        ...
        {/* Right Side Controls (For Desktop and Mobile/Tablet) */}
        <div className="flex items-center gap-2 sm:gap-4">
          
          {/* Action Buttons for Mobile/Tablet (Visible ONLY on screens < lg) */}
          <div className="flex lg:hidden items-center gap-2 sm:gap-3">
            <a 
              href="tel:+918586873357" 
              className="flex items-center gap-1 text-navy hover:text-medical transition-colors border border-navy/15 px-2.5 py-1.5 sm:px-3 sm:py-1.5 rounded-full font-bold text-xs sm:text-sm shrink-0 bg-gray-50/50 hover:bg-gray-100"
            >
            ...
            <button 
              onClick={openModal} 
              className="bg-medical hover:bg-medical/90 text-white font-bold text-xs sm:text-sm px-3.5 py-1.5 sm:px-4 sm:py-2 rounded-full transition-all shrink-0 shadow-md shadow-medical/15 whitespace-nowrap"
            >
              Apply Now
            </button>
          </div>

          {/* Action Buttons for Desktop (Visible ONLY on screens >= lg) */}
          <div className="hidden lg:flex items-center gap-4">
            <Link 
              href="https://wa.me/918586873357?text=Hi%20WCIEC%2C%20I%20want%20guidance%20for%20MBBS%20admission%20abroad." 
              className="flex items-center gap-2 text-[#25D366] font-semibold hover:text-[#1eb054] transition-colors text-sm xl:text-base"
            >
            ...
            <button onClick={openModal} className="btn-primary py-2 px-5 text-sm xl:px-6 xl:text-base">
              Apply Now
            </button>
          </div>
          ...
          {/* Mobile Menu Toggle */}
          <button 
            className="lg:hidden text-navy p-1 hover:bg-gray-100 rounded-full transition-colors"
      ...
      <AnimatePresence>
        {isMobileMenuOpen && (
          <motion.div
            ...
            className="lg:hidden bg-white mt-4 rounded-2xl shadow-2xl overflow-y-auto max-h-[80vh]"
          >
