...
export function PromDown({ title, subtitle, children, defaultOpen = false, className, titleClassName, subtitleClassName }: PromDownProps) {
  const [isOpen, setIsOpen] = useState(defaultOpen);

  return (
    <div className={cn("border border-[#E2E8F0] rounded-[24px] overflow-hidden shadow-sm transition-all", !className?.includes('bg-') && "bg-white", isOpen && "shadow-md border-primary/20", className)}>
